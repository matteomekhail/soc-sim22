#!/usr/bin/env python3
"""
Scenario 13: Verify SQL Injection detection by SOC stack.

Checks Suricata alerts, Wazuh alerts, and Loki logs to confirm
the attack was properly detected.
"""
...
LOG_FILE = os.path.join(os.path.dirname(__file__), "..", "logs", "sample_logs", "sqli_attack.jsonl")


def check_local_logs() -> dict:
    """Check locally generated attack logs."""
    results = {"total": 0, "ids_alerts": 0, "web_access": 0}
    ...
    with open(LOG_FILE) as f:
        for line in f:
            ...
            try:
                ...
                elif entry.get("event_type") == "web_access":
...
def check_wazuh_alerts() -> list[dict]:
    """Query Wazuh for SQL injection alerts."""
    try:
        api = WazuhAPI()
        if not api.check_connection():
            return []
        alerts = api.get_alerts(search="sql injection", limit=50)
        return alerts
...
def check_loki_logs() -> int:
    """Query Loki for scenario 13 logs."""
    try:
        siem = SIEMClient()
        result = siem.loki_query('{scenario="13-sqli"}', limit=100)
...
def main():
    print(f"""
{Fore.CYAN}╔══════════════════════════════════════════════════════════╗
║  WCACE Scenario 13: SQL Injection Detection Verification ║
╚══════════════════════════════════════════════════════════╝{Style.RESET_ALL}
""")
